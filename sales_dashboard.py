...
LEGEND = dict(bgcolor="rgba(0,0,0,0)", font=dict(color="#a09080"))
...
def extract_date(s):
    ...
    for p in parts:
        for fmt in ('%d/%m/%Y','%d/%m/%y'):
            try: return datetime.strptime(p, fmt)
            except: pass
    return None
...
for fp in sorted(os.listdir(BUNDLED)) if os.path.isdir(BUNDLED) else []:
...
curr_sales=curr_df['total'].sum(); curr_comm=curr_df['commission'].sum()
curr_bottles=curr_df['bottles'].sum(); prev_sales=prev_df['total'].sum()
pct=((curr_sales-prev_sales)/prev_sales*100) if prev_sales else 0
comm_rate=(curr_comm/curr_sales*100) if curr_sales else 0
arrow="↑" if pct>=0 else "↓"; dcls="up" if pct>=0 else "down"
...
with tab1:
    ...
    for fn in all_fortnights:
        ...
        if fn['date']:
            df['date']=fn['date']; rows.append(df)
    if not rows:
        ...
        fig.update_layout(**PLOT_BASE,title='Sales & Commission by Month',barmode='group',
                          yaxis_tickprefix='$',xaxis_tickangle=-30,height=400,
                          legend={**LEGEND,'orientation':'h','y':-0.2})
        ...
        fig2.update_layout(**PLOT_BASE,height=max(280,len(sku)*30),coloraxis_showscale=False,title='')
...
with tab2:
    ...
    with cl:
        ...
        fig3.update_layout(**PLOT_BASE,title=f'Sales by Customer — {current_label}',showlegend=False,height=450)
    ...
    with cr:
        ...
        fig4.update_layout(**PLOT_BASE,title='Top Customers',coloraxis_showscale=False,height=450)
        st.plotly_chart(fig4,use_container_width=True)
    with st.expander("Full customer table"):
        t=cust[['customer','total','pct']].copy()
        t.columns=['Customer','Sales','% Share']
        t['Sales']=t['Sales'].map(fmt); t['% Share']=t['% Share'].map(lambda x:f"{x*100:.1f}%")
...
with tab3:
    ...
    fig5=go.Figure()
    fig5.add_bar(x=smry['Fortnight'],y=smry['Sales'],name='Sales',marker_color=GOLD,marker_line_width=0,yaxis='y')
    fig5.add_scatter(x=smry['Fortnight'],y=smry['% Change'],name='% Change',mode='lines+markers',
                     line=dict(color=GREEN,width=2),marker=dict(size=7,color=GREEN),yaxis='y2')
    l5={**PLOT_BASE,
        'yaxis':dict(title='Sales ($)',titlefont_color=GOLD,tickprefix='$',gridcolor='#1e1a14',linecolor='#2a2520'),
        'yaxis2':dict(title='% Change',titlefont_color=GREEN,overlaying='y',side='right',
                      ticksuffix='%',gridcolor=None,zeroline=True,zerolinecolor='#2a2520')}
    fig5.update_layout(**l5,title='Sales & Period-on-Period % Change',xaxis_tickangle=-30,height=400,
                       legend={**LEGEND,'orientation':'h','y':-0.22})
    st.plotly_chart(fig5,use_container_width=True)

    cr2=smry[smry['Fortnight']==current_label].iloc[0]
    pr2=smry[smry['Fortnight']==compare_label].iloc[0]
    ds=cr2['Sales']-pr2['Sales']; dp=(ds/pr2['Sales']*100) if pr2['Sales'] else 0
    dc=cr2['Commission']-pr2['Commission']; db=cr2['Bottles']-pr2['Bottles']
    ...
    st.markdown("<div class='section-rule'></div>",unsafe_allow_html=True)
    t2=smry.copy()
    t2['Sales']=t2['Sales'].map(fmt); t2['Commission']=t2['Commission'].map(fmt)
...
with tab4:
    ...
    rs=curr_df.groupby('rep').agg(Sales=('total','sum'),Commission=('commission','sum'),
        Bottles=('bottles','sum'),Accounts=('customer','nunique')).reset_index().sort_values('Sales',ascending=False)
    ...
    with cl2:
        ...
        fig6.update_layout(**PLOT_BASE,title='Sales by Rep',height=260,showlegend=False)
    ...
    with cr2:
        ...
        fig7.update_layout(**PLOT_BASE,title='Sales by SKU',coloraxis_showscale=False,height=260)
    ...
    det['Total']=det['Total'].map(fmt); det['Commission']=det['Commission'].map(fmt)
